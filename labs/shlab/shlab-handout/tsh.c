...
void eval(char *cmdline) 
{
    ...
    if(!builtin_cmd(argv)){
        ...
        if(bg) printf("[%d] (%d) %s", pid2jid(pid), pid, buf);
        else waitfg(pid);
...
void sigchld_handler(int sig) 
{
    pid_t pid;
    int olderrno = errno;
    errno = 0; // This handler may cause EINTP, so we need save this errno and set it as 0 in this handler.
    int status;
...
void sigtstp_handler(int sig) 
{
    ...
    if(pid){
        struct job_t* job = getjobpid(jobs, pid);
        if(job->state == FG) kill(-pid, SIGTSTP);
    }
    return;
}
